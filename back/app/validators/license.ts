...
export const trackSeparationEnum =
  ['full_mix', 'stems', 'instrumental_only', 'vocal_only', 'acapella'] as const
export const platformsEnum = ['tiktok', 'youtube', 'instagram', 'twitch', 'facebook', 'snapchat'] as const
...
export const territoriesEnum = [
  'US', 'FR', 'DE', 'GB', 'IT', 'ES', 'JP', 'CN', 'BR', 'CA', 'AU', 'NZ', 'SG', 'HK', 'IN', 'MX',
  'WORLDWIDE'
] as const
...
const baseLicenseSchema = vine.object({
  ...
  // === AUDIO FORMATS ===
  audioFormats: vine
    .array(vine.enum(audioFormatsEnum))
    .nullable()
    .optional(),
  ...
  allowedPlatforms: vine
    .array(vine.enum(platformsEnum))
    .nullable()
    .optional(),
  ...
  allowedTerritories: vine
    .array(vine.enum(territoriesEnum))
    .nullable()
    .optional(),
