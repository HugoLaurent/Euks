...
function getDefaultLicenseId(cards) {
  ...
  return cheapestCard?.selectionId ?? cards[0]?.selectionId;
}
...
function PurchaseModal({ isOpen, onClose, onPlay, track, language = "en" }) {
  ...
  const [prevDefaultLicenseId, setPrevDefaultLicenseId] =
    useState(defaultLicenseId);

  // Reset the selection when the modal switches to another track's default
  // license. Adjusting state during render (instead of in an effect) avoids a
  // wasted render pass and is React's recommended pattern for this case.
  if (defaultLicenseId !== prevDefaultLicenseId) {
    setPrevDefaultLicenseId(defaultLicenseId);
  ...
  const selectedLicense =
    licenseCards.find(
      (card) => card.selectionId === resolvedSelectedLicenseId,
    ) ?? licenseCards[0];
            ...
            {!track.licenseError && licenseCards.length > 0 ? (
              <div className="flex flex-col gap-4 lg:gap-5">
                {licenseCards.map((card) => {
                  ...
                  const isSelected =
                    card.selectionId === resolvedSelectedLicenseId;
